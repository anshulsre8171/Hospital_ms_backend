    ...
    <>
      <div className="container-fluid">
        <div className="row">
          <div
            className={
              data == false? "col-lg-2 px-0  d-none d-lg-block dashboard-left-container" : "col-lg-1 px-0  d-none d-lg-block dashboard-left-container"
            }>
            <div
              className={data == false ? "left-top bg-dark d-flex justify-content-between align-items-center px-3" : "left-top bg-dark d-flex  align-items-center "
              } >
            ...
            <div className="left-bottom text-light p-3">
              <ul className="navbar-nav mx-auto">
                {sideNav?.map((items: any,index:any) => (
                  <>
                    <li className="nav-item mb-2 rounded-pill">
                      <Link key={index}
                        className={data == false? " rounded-pill d-flex align-items-center nav-link px-3 py-2" : " rounded-pill d-flex align-items-center nav-link px-1 py-2 text-center"
                        } href={items.link} >
                        <span className={data == false? "d-block my-span me-2 fs-4" : "d-block my-span mx-auto  fs-4 " }>
                          {items.icon}
                        </span>
                        <span className={data == false ? "d-block px-2 " : "d-none"} >{items.title}{" "}</span>
                      </Link>
                    </li>
                  </>
                ))}
